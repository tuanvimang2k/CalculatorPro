...
const Calculator = ({ navigation }) => {
  ...
  useEffect(() => {
    if (isDarkMode) {
      ...
      setImgLink(require('../../assets/imgs/sun.png'))
    }
  }, [isDarkMode]);
  ...
  return (
    <View style={[styles.lightContainer, isDarkMode ? { backgroundColor: 'black' } : null]}>
      ...
      <TouchableOpacity onPress={() => { toggleImage(); toggleDarkMode(); }}>
        {isImage1 ? (
          <Image
            source={imgLink} // Replace with the path to your first image
            style={styles.image}
          />
        ) : (
          <Image
            source={imgLink} // Replace with the path to your second image
            style={styles.image}
          />
        )}
      </TouchableOpacity>
